...
imgdir = r"/workspace/YOLOv5-Lite-master/data/checkreal0/"
imgplist = glob.glob("{}/*/JPEGImages/*.jpg".format(imgdir))
for imgp in tqdm(imgplist):
    dirname = os.path.dirname(imgp)
    labeldir = dirname.replace('JPEGImages','labels20')
    if not os.path.exists(labeldir):
        os.makedirs(labeldir)
    labelp = (imgp[:-3]+'txt').replace('JPEGImages','labels20')
