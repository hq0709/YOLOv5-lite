...
std::vector<float> V5lite::prepareimg(cv::Mat &src_img){
    std::vector<float> result(1 * IMAGE_WIDTH * IMAGE_HEIGHT * INPUT_CHANNEL);
    float *data = result.data();
    if (!src_img.data)
    ...
    std::vector<cv::Mat> split_img = {
            ...
            cv::Mat(IMAGE_HEIGHT, IMAGE_WIDTH, CV_32FC1, data + 0)
    };
    cv::split(flt_img, split_img);
    return result;
}
...
bool V5lite::GetRst(cv::Mat &src_img, std::vector<V5lite::DetectRes> & res)
{
    int index = 0;
    int batch_id = 0;
    float total_time = 0;
    ...
    vec_Mat[0] = src_img.clone();

    auto t_start_pre = std::chrono::high_resolution_clock::now();
    std::cout << "prepareImage" << std::endl;
    std::vector<float>curInput = prepareimg(src_img);
    auto t_end_pre = std::chrono::high_resolution_clock::now();
    float total_pre = std::chrono::duration<float, std::milli>(t_end_pre - t_start_pre).count();
    std::cout << "prepare image take: " << total_pre << " ms." << std::endl;
    total_time += total_pre;
    ...
    std::cout << "post process" << std::endl;
    auto r_start = std::chrono::high_resolution_clock::now();
    auto *out = new float[outSize * 1];
    cudaMemcpyAsync(out, buffers[1], bufferSize[1], cudaMemcpyDeviceToHost, stream);
    cudaStreamSynchronize(stream);
    res.swap(postProcess(vec_Mat, out, outSize)[0]);
    auto r_end = std::chrono::high_resolution_clock::now();
    float total_res = std::chrono::duration<float, std::milli>(r_end - r_start).count();
    std::cout << "Post process take: " << total_res << " ms." << std::endl;
    delete[] out;
